...
function parseOllamaLine(
  ...
  if (typeof parsed !== 'object' || parsed === null) {
    throw new BadGatewayException(
      'Fragmento inválido recebido do modelo',
    );
  }
...
export class OllamaProvider implements ModeloProvider {
  ...
  async gerar(
    ...
    const baseUrl =
      this.config.getOrThrow<string>('OLLAMA_BASE_URL');
    ...
    try {
      const response =
        await this.http.axiosRef.post<OllamaChatResponse>(
          `${baseUrl}/api/chat`,
          {
            model,
            messages: [
  ...
  async *gerarStream(
    ...
    const response =
      await this.http.axiosRef.post<NodeJS.ReadableStream>(
        `${baseUrl}/api/chat`,
        {
          model,
          messages: [
